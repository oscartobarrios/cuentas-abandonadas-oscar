...
@Component({
  selector: 'app-certificados-sin-cargue',
  templateUrl: './certificados-sin-cargue.component.html',
  styleUrls: ['./certificados-sin-cargue.component.css']
})
export class CertificadosSinCargueComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.idTipoCargue = this.route.snapshot.params['id'];
    this.idOrganizacion = this.route.snapshot.params['entidad'];
  ...
  getCertificaciones(){
    this._getarchivousecase.ListarCertificacionesSinCargue(this.idTipoCargue, this.idOrganizacion).subscribe((ResultData) => {
  ...
  onSubmit(){
    if(this.certificacionesForm.valid)
    {
      ...
      if(!repetida){
        const data = {
          idTipoCargue: this.idTipoCargue,
        ...
        console.log(data)
        this._getarchivousecase.CargarCertificadoSinCargue(data).subscribe((ResponseData) => {
          Swal.close()
